...
<hibernate-mapping package="${classPackage}">
    <class name="${className}" table="${tableName}" lazy="false">
        ...
        <property name="${javaName}" column="${columnName}" type="${javaType}" not-null="${nullable}" length="${dataLength}" unique="false" optimistic-lock="true" lazy="false" generated="never"/>
        </#list>
    </class>
</hibernate-mapping>